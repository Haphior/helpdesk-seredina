...
export function NotificationSettings() {
  ...
  return (
    <div className="px-8 py-7">
      ...
      {preferences === null && <p className="text-sm text-slate-500">{t('common.loading')}</p>}
          ...
          <div className="divide-y divide-slate-100">
            {preferences.map((p) => (
              <div key={p.eventType} className="grid grid-cols-[1fr_90px_90px] items-center gap-3 px-5 py-3.5">
                <span className="text-[13.5px] text-slate-700">{t(`notificationSettings.events.${p.eventType}`, { defaultValue: p.label })}</span>
                <input
                  type="checkbox"
                  checked={p.inApp}

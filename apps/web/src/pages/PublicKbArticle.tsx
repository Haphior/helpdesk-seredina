  ...
  function load() {
    ...
    apiGet<PublicKbArticle>(`/public/${tenantSlug}/kb-articles/${slug}`, { headers: kbAccessHeaders(tenantSlug) })
      ...
      .catch((err) => {
        ...
        setError(err instanceof ApiError ? err.message : t('publicKb.articleNotFound'));
      });
  }
    ...
    <div className="min-h-screen bg-slate-50 px-6 py-10">
      <div className="mx-auto max-w-2xl">
        ...
        >
          ...
          {t('publicKb.allArticles')}
        </Link>

        {error && <p className="text-sm text-rose-600">{error}</p>}
        {!article && !error && <p className="text-sm text-slate-500">{t('common.loading')}</p>}

        {article && (
          <Card className="!p-6">
            <h1 className="mb-1 text-[22px] font-extrabold tracking-tight text-slate-900">{article.title}</h1>
            <p className="mb-4 text-[12.5px] text-slate-400">{t('publicKb.updated', { when: formatDateTime(article.updatedAt) })}</p>

...
- Every tenant-scoped request/job runs inside an explicit Prisma **interactive
  transaction** (`withTenantTx()`, `packages/db/src/tenant-context.ts`), whose first
...
- A Prisma **Client Extension** (`packages/db/src/prisma.ts`, not the deprecated
...
`packages/db/prisma/rls/policies.sql`) that runs with the table owner's privileges
...
- Every new tenant-scoped model added in later phases needs both an RLS policy in
  `packages/db/prisma/rls/policies.sql` and an entry in `TENANT_SCOPE_FIELD`
  (`packages/db/src/prisma.ts`) — there is no single place that makes this automatic,

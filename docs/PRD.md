...
| 2 — Configurability & SLA | Alert ingestion (NOC/SOC) ✅, custom fields ✅, hardware/equipment catalog ✅, IT processes/procedures ✅, outbound webhooks ✅, macros ✅, SLA engine ✅, reporting v1 + dashboard ✅, Change Enablement ✅, Release Management ✅, Problem Management ✅, Service Catalog ✅, Service Configuration Management ✅, self-service portal + knowledge base ✅ (pulled forward from Phase 3), collision detection + ticket merge + bulk actions ✅, on-call scheduling + SLA escalation chains ✅, saved views per agent ✅, notification preferences per agent ✅, AI cost transparency ✅ (differentiator 1 of 3), full data portability ✅ (differentiator 2 of 3), public status page ✅ (differentiator 3 of 3), ticket status configuration ✅, first-run onboarding tour ✅, self-hosted setup script + startup validation ✅ — every named Phase 2 item complete, all three differentiators shipped; only Sentry (deferred until cloud infra exists) remains (see ROADMAP.md for the full expanded list; integrations, not a plugin platform, is the decided extensibility path) | 🚧 In progress |
| 3 — AI depth | RAG (pgvector) ✅, shared AI tool catalog + `AutonomyPolicy` + `AiAgentRun` audit trail ✅, `apps/mcp-server` (stdio + Streamable HTTP) ✅, second LLM provider (OpenAI) + local option (Ollama) ✅, full autonomous tool-use loop ✅ — every named Phase 3 item complete | ✅ Done |
| 4 — Cloud hardening | Self-hosted single-tenant enforcement ✅, BYO AI key ✅, custom roles ✅, first CI pipeline + RLS fuzz tests ✅, multi-replica audit ✅ (rate limiter fixed; "WS fanout" corrected as stale, never real; email-poll double-polling still open), Grafana Alerting integration ✅, embeddable web chat widget ✅, tenant-configurable UI theme system ✅ (default "Meet in the Middle" + original "Refined" kept), tenant branding/white-label v1 ✅ (logo + accent color on the self-service portal and status page), Telegram channel ✅ (bot-based, no OAuth blocker), CSAT surveys ✅, Zabbix integration ✅ (script-based, no new backend code), Phase 5 endpoint agents ✅ (inventory-only by design, not a stepping stone — see Phase 5 below), Slack/Teams chat notifications ✅ (bring-your-own webhook URL, same wire shape for both platforms confirmed against their current live docs — no OAuth app), WhatsApp — re-scoped the same way, not started (needs the tenant's own Meta Business verification, a genuinely bigger lift than Telegram's), console i18n v1/v1.1 ✅ (`react-i18next`, per-user locale preference, English + Spanish covering auth/nav/dashboard/Tickets Queue/Ticket Detail — the rest of the console still backlog, ADR 0049); plan-tier scaffolding deliberately deferred (real pricing is a business decision) | 🚧 In progress |
| 5 — Endpoint agents | ✅ Done, inventory-only by design (not a stepping stone) — unsigned Go agent for Windows/macOS/Linux in its own repo ([Haphior/seredina-agent](https://github.com/Haphior/seredina-agent), `docs/adr/0068-go-endpoint-agent.md`; it replaced the original Node.js `apps/agent`) that installs as a service, per-device enrollment tokens, hardware/software/OS/disk-encryption/AV inventory into the CMDB. Remote execution/deployment (tiers 2/3) and a signed installer are deliberately not planned: both carry ongoing costs (a code-signing cert, an Apple Developer membership, an audited remote-execution surface) an unfunded open-source project has no way to responsibly absorb; revisit only if a funded fork/sponsor changes that. Desktop-only (not mobile MDM) | ✅ Done |
...
automation engine, SSO/SAML, data residency. Console i18n has a real v1 (see Phase
4 below) rather than being out of scope; full page coverage remains backlog. Also: **NOC/SOC will

...
| 3 — AI depth | RAG (pgvector) grounding `suggestReply` ✅, shared AI tool catalog + `AutonomyPolicy` + `AiAgentRun` audit trail + `apps/mcp-server` (stdio) ✅, second LLM provider (OpenAI) + local option (Ollama) ✅, full autonomous tool-use loop ✅ — only HTTP/SSE transport for the MCP server remains | 🚧 In progress |

...
| 4 — Cloud hardening | Self-hosted single-tenant enforcement ✅, BYO AI key ✅, custom roles ✅, first CI pipeline + RLS fuzz tests ✅, multi-replica audit ✅ (rate limiter fixed; "WS fanout" corrected as stale, never real; email-poll double-polling still open), widget/WhatsApp channels, pre-built integrations — plan-tier scaffolding deliberately deferred (real pricing is a business decision) | 🚧 In progress |

...
| 3 — AI depth | RAG (pgvector) grounding `suggestReply` ✅ (local embeddings, pluggable adapter), shared AI tool catalog + `AutonomyPolicy` + `AiAgentRun` audit trail + `apps/mcp-server` (stdio) ✅, HTTP/SSE transport + full autonomous copilot loop + second LLM provider still pending | 🚧 In progress |
